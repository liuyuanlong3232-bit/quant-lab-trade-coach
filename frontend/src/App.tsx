...
import { ScreenshotCandidate, type AccountCandidate } from './ScreenshotCandidate'
...
function HoldingsPage({ summary, onUpdated }: { summary: CoachSummary; onUpdated: () => void }) {
  ...
  const applyScreenshotCandidate = (values: AccountCandidate) => setForm((current) => ({ ...current, ...Object.fromEntries(Object.entries(values).filter(([, value]) => value.trim())) }))
...
function SourcesPage({ summary, onUpdated }: { summary: CoachSummary; onUpdated: () => void }) {
  const auto = summary.auto_refresh
  const lastAuto = auto.last_auto_refresh
  return <><PageIntro eyebrow="PROVENANCE / HEALTH" title="数据来源与错误">来源、交易所语义、时间、新鲜度和故障原因全部可见。`EVENT_CALENDAR_UNAVAILABLE` 保持缺失，不被改成 GREEN。提醒适配器也在本页显示真实配置状态。</PageIntro><Panel title="事实层健康" eyebrow="SOURCE MATRIX"><div className="source-summary"><Metric label="行情模式证据" value={statusLabel(summary.market_regime.evidence_status)} detail={`${summary.market_regime.available_symbols.length} 个因子可用`} tone={statusTone(summary.market_regime.evidence_status)} /><Metric label="VPS风险事实" value={statusLabel(summary.vps.status)} detail={summary.vps.reason_codes.join(' / ') || '无故障码'} tone={statusTone(summary.vps.status)} /><Metric label="Prediction Gate" value={statusLabel(summary.vps.prediction_gate_status)} detail="只消费时点事实" tone={statusTone(summary.vps.prediction_gate_status)} /></div></Panel><Panel title="自动数据刷新" eyebrow="EVIDENCE-GATED SCHEDULE"><div className="source-summary"><Metric label="调度状态" value={statusLabel(auto.status)} detail={auto.reason_codes.join(' / ') || `${auto.timezone} · 常驻运行`} tone={statusTone(auto.status)} /><Metric label="最近自动刷新" value={lastAuto?.status || '尚未执行'} detail={lastAuto ? `${lastAuto.schedule_key || ''} · ${formatTimestamp(lastAuto.event_at || null)}` : '缺少可靠交易日证据时不会联网'} tone={statusTone(lastAuto?.status || 'UNKNOWN')} /><Metric label="下次计划" value={formatTimestamp(auto.next_planned_at)} detail={`固定点 ${auto.points.join(' / ')}；错过不回补`} tone={auto.next_planned_at ? 'ready' : 'neutral'} /></div><small>仅经哈希校验的 Tushare A股交易日历可放行；周末、节假日或日历未知均跳过。仅刷新公开数据，不生成订单、不自动交易。</small></Panel><NotificationPanel summary={summary} onUpdated={onUpdated} /><Panel title="全部标的来源" eyebrow="MARKET DATA CONTRACT"><div className="table-scroll"><table><thead><tr><th>标的</th><th>市场/交易所</th><th>提供方标识</th><th>采用源</th><th>行情时间</th><th>新鲜度</th><th>主备协调</th><th>原因</th></tr></thead><tbody>{summary.instruments.map((item) => <tr key={item.symbol}><td><b>{item.label}</b><small>{item.symbol}</small></td><td>{item.venue}</td><td><code>{item.provider_symbol}</code><small>{item.contract_semantics}</small></td><td>{item.selected.source || '未记录'}</td><td>{formatTimestamp(item.selected.exchange_time)}</td><td><StatusBadge status={item.selected.status} /></td><td>{item.reconciliation_status}</td><td>{item.selected.reason_codes.length ? item.selected.reason_codes.join(' / ') : '无'}</td></tr>)}</tbody></table></div></Panel><div className="source-detail-grid">{summary.instruments.map((item) => <SourceDetail key={item.symbol} item={item} />)}</div><Panel title="VPS受限事实链" eyebrow="EVENT CALENDAR / PREDICTION GATE / RISK"><div className="vps-card"><div><StatusBadge status={summary.vps.status} /><strong>{summary.vps.risk_level || '未发布风险等级'}</strong></div><div className="vps-values"><span>事件日历：<b>{summary.vps.macro_event_gate || 'MISSING'}</b></span><span>Prediction Gate：<b>{statusLabel(summary.vps.prediction_gate_status)}</b></span><span>有效至：<b>{formatTimestamp(summary.vps.valid_until)}</b></span></div><ListItems items={summary.vps.reason_codes} /><small>来源：{summary.vps.source_ref || '未配置本地 VPS 导出路径'}。本地消费不会修改 VPS。</small></div></Panel></>
}
