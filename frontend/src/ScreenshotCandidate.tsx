...
export function ScreenshotCandidate({ onApplyAccount, onUpdated, plannedCashOut = 0 }: { onApplyAccount: (candidate: AccountCandidate) => void; onUpdated: () => void; plannedCashOut?: number }) {
  ...
  const confirmAccount = async () => {
    if (!checked) { setMessage('请先逐项核对，并勾选“我确认截图中的账户数据”。'); return }
    const shares = Number(account.shares)
    const avgCost = Number(account.avg_cost)
    const availableCash = Number(account.available_cash)
    const totalAssets = Number(account.total_assets)
    if (!account.shares || !account.avg_cost || !account.available_cash || !account.total_assets || !Number.isInteger(shares) || shares < 0 || shares % 100 !== 0 || avgCost < 0 || availableCash < 0 || totalAssets < 0) {
      setMessage('持仓、成本、可用现金或总资产缺失/非法；请核对后再确认。持仓必须是100股整数手。'); return
    }
    try {
      await postCoach('account/confirm', { shares, avg_cost: avgCost, available_cash: availableCash, total_assets: totalAssets, planned_cash_out: plannedCashOut, note: '用户核对券商持仓截图后确认' })
      setMessage('账户事实已更新；没有连接券商，也没有发送订单。'); onUpdated()
    } catch (error) { setMessage(error instanceof Error ? error.message : '账户更新失败') }
  }
  ...
  return <section className="screenshot-candidate">
    <div className="screenshot-candidate__intro"><div><b>从券商截图自动建立候选</b><p>图片只在浏览器本地处理，不上传、不保存；OCR文本仅驻留当前页面内存。首次识别可能下载简体中文模型。</p></div><div className="screenshot-source-actions"><label className="file-button">从相册选择<input type="file" accept="image/*" onChange={(event) => selectFile(event.target.files?.[0] || null)} /></label><label className="file-button file-button--secondary">拍照<input type="file" accept="image/*" capture="environment" onChange={(event) => selectFile(event.target.files?.[0] || null)} /></label></div></div>
    {file && <div className="screenshot-candidate__workspace"><div className="screenshot-preview"><img src={preview} alt="仅本地显示的券商截图预览" /><small>{file.name} · 原图未上传</small>{progress !== null && <progress max="100" value={progress}>{progress}%</progress>}</div><div className="candidate-editor"><div className="candidate-kind"><button type="button" className={kind === 'holding' ? 'is-active' : ''} onClick={() => { setKind('holding'); setChecked(false) }}>持仓/资产截图</button><button type="button" className={kind === 'trade' ? 'is-active' : ''} onClick={() => { setKind('trade'); setChecked(false) }}>成交截图</button></div>
